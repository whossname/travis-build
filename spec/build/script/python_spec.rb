...
describe Travis::Build::Script::Python, :sexp do
  ...
  it 'sets up the python version (pypy)' do
    ...
    should include_sexp [:cmd,  "curl -sSf --retry 2 -o pypy.tar.bz2 ${archive_url}", echo: true]
  end
  ...
  it 'sets up the python version (pypy-5.3.1)' do
    ...
    should include_sexp [:cmd,  "curl -sSf --retry 2 -o pypy-5.3.1.tar.bz2 ${archive_url}", echo: true]
  ...
  it 'sets up the python version (pypy3.3-5.2-alpha1)' do
    ...
    should include_sexp [:cmd,  "curl -sSf --retry 2 -o pypy3.3-5.2-alpha1.tar.bz2 ${archive_url}", echo: true]
    should include_sexp [:cmd,  "rm pypy3.3-5.2-alpha1.tar.bz2"]
  end

  it 'sets up the python version (pypy3)' do
    data[:config][:python] = 'pypy3'
    should include_sexp [:cmd,  "curl -sSf --retry 2 -o pypy3.tar.bz2 ${archive_url}", echo: true]
