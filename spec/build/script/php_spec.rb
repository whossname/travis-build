...
describe Travis::Build::Script::Php, :sexp do
  ...
  describe 'installs php nightly' do
    ...
    xit { should include_sexp [:cmd, 'curl -sSf --retry 2 -o archive.tar.bz2 $archive_url && tar xjf archive.tar.bz2 --directory /', timing: true] }
  end
  ...
  describe 'when desired PHP version is not found' do
    ...
    it 'installs PHP version on demand' do
      ...
      expect(sexp).to include_sexp [:cmd, "curl -sSf --retry 2 -o archive.tar.bz2 $archive_url && tar xjf archive.tar.bz2 --directory /", echo: true, timing: true]
    end
  end
